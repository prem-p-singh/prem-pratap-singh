...
tldr:
  - "Cross-kingdom RNA can promote, not only suppress, plant-microbe interactions, including arbuscular mycorrhiza development."
  - "Climate extremes can override mechanistic findings by reshaping pathogens, microbiomes, and plant stress in the field."
  - "Useful research pairs RNA-level molecular signaling with field-scale climate variability."
  - "For grapevine systems, this elevates microbiome and mycorrhiza work alongside virus biology."
---
...
Two developments (as covered in the provided sources) sharpen the research field:
...
**Practical implication:** if RNA-mediated cross‑kingdom effects can tune symbiosis, then RNA-based tools (or strategies that use endogenous RNA exchange) might one day be used to steer plant-associated communities toward outcomes that improve plant performance, potentially including stress tolerance that indirectly affects disease.

### 2) Climate extremes positioned as a central driver of pathogen, microbiome, plant dynamics 
The Nature-linked piece on climate extremes emphasizes that extremes influence plant pathogens, microbiomes, and plant health. The key 'change' here is not a single technique, but the consolidation of evidence that extremes are not just background noise, they can be primary determinants of which microbes thrive, how pathogens persist, and how plants respond.
...
- Microbial and pathogen processes operate from millimeters (biofilms, root interfaces) to meters (vine-to-vine spread) to field scales (vector movement, weather patterns).
