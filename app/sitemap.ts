...
export default function sitemap(): MetadataRoute.Sitemap {
  ...
  // Dynamic method case studies
  const methodPages: MetadataRoute.Sitemap = getAllMethodPosts().map((post) => ({
    url: `${baseUrl}/methods/${post.slug}`,
    lastModified: new Date(post.date),
    changeFrequency: "monthly" as const,
    priority: 0.7,
  }));

  return [...staticPages, ...blogPages, ...dataPages, ...methodPages];
}
